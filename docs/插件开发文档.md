...
// 插件配置加载
window.webUtils.loadPluginConfig(configPath: string)

// 文件路径获取
window.webUtils.getPathForFile(file: File): string
```
...
module.exports = {
  ...
  items: [
    ...
    },
    {
      ...
      weight: 90,
      onEnter: (params, api) => {
        api.ipcRouter.appLaunchApp("calc.exe");
      },
    },
  ],
};
...
onEnter: async (params, api) => {
  try {
    const result = await api.ipcRouter.filesystemReadFile(filePath);
    // 处理结果
  } catch (error) {
    console.error("文件读取失败:", error);
    api.ipcRouter.logError(`插件执行失败: ${error.message}`);
  }
};
...
// 使用系统日志
api.ipcRouter.logInfo("插件信息日志");
api.ipcRouter.logError("插件错误日志");
```
