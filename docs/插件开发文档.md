...
```javascript
// 获取插件设置 - 正确的调用方式
const settings = await naimo.ipcRouter.storeGet(
  "pluginSettings.your-plugin-id"
);

// 设置插件配置
await naimo.ipcRouter.storeSet("pluginSettings.your-plugin-id", settingsData);

// 删除配置
await naimo.ipcRouter.storeDeleteKey("pluginSettings.your-plugin-id");
```
...
通过 `naimo.ipcRouter` 可以访问所有主进程功能：

```javascript
// 应用操作
naimo.ipcRouter.appLaunchApp(path: string)
naimo.ipcRouter.appGetInstalledApps()

// 文件系统操作
naimo.ipcRouter.filesystemReadFile(path: string)
naimo.ipcRouter.filesystemWriteFile(path: string, content: string)
naimo.ipcRouter.filesystemGetPluginsDirectory()

// 窗口操作
naimo.ipcRouter.windowCreateWebPageWindow(parentId: number, url: string)
naimo.ipcRouter.windowShow(id: number)
naimo.ipcRouter.windowHide(id: number)
naimo.ipcRouter.windowClose(id: number)
naimo.ipcRouter.windowMinimize(id: number)
naimo.ipcRouter.windowMaximize(id: number)

// 热键操作
naimo.ipcRouter.hotkeyRegister(accelerator: string)
naimo.ipcRouter.hotkeyUnregister(accelerator: string)

// 日志操作
naimo.ipcRouter.logInfo(message: string)
naimo.ipcRouter.logError(message: string)
naimo.ipcRouter.logWarn(message: string)
naimo.ipcRouter.logDebug(message: string)

// 存储操作
naimo.ipcRouter.storeGet(key: string)
naimo.ipcRouter.storeSet(key: string, value: any)
naimo.ipcRouter.storeDeleteKey(key: string)
naimo.ipcRouter.storeClear()

// 搜索操作
naimo.ipcRouter.searchGetInstalledApps()
naimo.ipcRouter.searchGetSearchResults(query: string)
```
...
if (naimo && naimo.ipcRouter) {
  pluginSettings =
    (await naimo.ipcRouter.storeGet("pluginSettings.your-plugin-id")) || {};
}
```
...
```javascript
// 读取文件
const content = await naimo.ipcRouter.filesystemReadFile("/path/to/file.txt");

// 写入文件
await naimo.ipcRouter.filesystemWriteFile("/path/to/file.txt", "content");

// 获取插件目录
const pluginDir = await naimo.ipcRouter.filesystemGetPluginsDirectory();
```
...
// ✅ 在HTML中正确使用API
// 基础功能使用 electronAPI
const settings = await naimo.ipcRouter.storeGet("pluginSettings.my-plugin");
