...
const handleClick = async () => {
  ...
  // 记录日志
  naimo.log.info("按钮被点击", { count: localCount.value });
};
...
const getAppInfo = async () => {
  try {
    ...
    const name = await naimo.router.appGetName();
...
const saveUserSettings = async () => {
  try {
    await naimo.router.storeSet("theme", "dark");
    await naimo.router.storeSet("language", "zh-CN");
    console.log("设置已保存");
  } catch (error) {
...
const version = await naimo.router.appGetVersion();
const name = await naimo.router.appGetName();
const path = await naimo.router.appGetAppPath();
const userDataPath = await naimo.router.appGetUserDataPath();
const isPackaged = await naimo.router.appIsPackaged();
const systemInfo = await naimo.router.appGetSystemInfo();
const config = await naimo.router.appGetConfig();

// 应用控制
await naimo.router.appShowAbout();
await naimo.router.appRestart();
await naimo.router.appQuit();
```
...
```typescript
// 文件选择
const files = await naimo.router.filesystemSelectFile({
  title: "选择文件",
  filters: [
...
// 文件夹选择
const folders = await naimo.router.filesystemSelectFolder({
  title: "选择文件夹",
});
...
```typescript
// 获取日志
const logs = await naimo.router.logGetLogs();
const rawContent = await naimo.router.logGetRawLogContent();
const logInfo = await naimo.router.logGetLogInfo();

// 日志操作
await naimo.router.logClearLogs();
const exportedTxt = await naimo.router.logExportLogs("txt");
const exportedJson = await naimo.router.logExportLogs("json");
```
...
```typescript
// 获取配置
const theme = await naimo.router.storeGet("theme");
const allConfig = await naimo.router.storeGet(); // 获取所有配置
...
// 删除配置
await naimo.router.storeDeleteKey("theme");
...
// 打开日志查看器
await naimo.router.windowOpenLogViewer();
```
...
const handleApiCall = async () => {
  try {
    const result = await naimo.router.appGetVersion();
...
// ✅ 推荐：使用 naimo.log 记录日志
const logUserAction = (action: string) => {
  naimo.log.info("用户操作", { action, timestamp: Date.now() });
};
</script>
```
...
A: 使用 `naimo.log` 记录日志，或者在主进程中使用 `console.log` 查看调用情况。
