...
function getPluginHelperTypes(project) {
  ...
  if (ubrowserType) {
    ...
    console.log('   ✓ 从源码成功提取 UBrowser 类型');
  } else {
    console.warn('   ⚠️  警告: 无法从源码提取 UBrowser 类型，将跳过');
  }
        ...
        for (const objProp of objProps) {
          if (objProp.getKind() === SyntaxKind.PropertyAssignment) {
            ...
            if (methodInit && methodInit.getKind() === SyntaxKind.ArrowFunction) {
              ...
              try {
                // 首先尝试获取显式的返回类型注解
                const returnTypeNode = methodInit.getReturnTypeNode();
                if (returnTypeNode) {
