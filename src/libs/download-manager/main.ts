...
export class DownloadManagerMain {
  ...
  constructor(storageProvider?: StorageProvider) {
    ...
    console.log(
      "DownloadManagerMain 构造函数，存储提供者:",
      this.storageProvider ? "已设置" : "未设置"
    );
    // 从存储中恢复下载任务
  ...
  private loadDownloadsFromStorage(): void {
    ...
    try {
      const savedDownloads = this.storageProvider.get<DownloadStatus[]>(
        "downloads",
        []
      );
  ...
  private saveDownloadsToStorage(): void {
    ...
    try {
      const downloadsArray = Array.from(this.downloads.values()).map(
        (download) => ({
          ...download,
      ...
      console.log(
        `正在保存 ${downloadsArray.length} 个下载任务到存储:`,
        downloadsArray
      );
    ...
    for (const [downloadId, download] of this.downloads.entries()) {
      if (download.metadata?.persistedRestoreData) {
        try {
              ...
              callbacks: {
                ...
                onDownloadProgress: async (data) => {
                  const validProgress = Math.max(
                    0,
                    Math.min(100, Math.round(data.percentCompleted || 0))
                  );
          ...
          console.log(
            `持久化下载恢复成功: ${downloadId} -> ${restoredDownloadId}`
          );
        } catch (error) {
  ...
  private sendToRenderer(channel: string, data: any): void {
    try {
      ...
      if (this.messageBroadcaster) {
        console.log(
          `[DownloadManager] 使用自定义广播函数发送事件: ${channel}`,
          data.id ? `(ID: ${data.id})` : ""
        );
      ...
      if (allWindows.length > 0) {
        console.log(
          `[DownloadManager] 向 ${allWindows.length} 个 BrowserWindow 广播事件: ${channel}`,
          data.id ? `(ID: ${data.id})` : ""
        );
      ...
      if (this.mainWindow && !this.mainWindow.isDestroyed()) {
        console.log(
          `[DownloadManager] 向主窗口发送事件: ${channel}`,
          data.id ? `(ID: ${data.id})` : ""
        );
  ...
  private setupIpcHandlers(): void {
    ...
    ipcMain.handle("start-download", async (event, params: DownloadParams) => {
      if (!this.isReady()) {
        throw new Error(
          "下载管理器未准备好。请先调用 initialize() 或 setMainWindow() 方法。"
        );
      }
    ...
    ipcMain.handle("pause-download", (event, downloadId: string) => {
      ...
      try {
        ...
        if (download.status !== "downloading") {
          console.log(
            `下载 ${downloadId} 当前状态为 ${download.status}，无需暂停`
          );
          return true;
        }
    ...
    ipcMain.handle("resume-download", async (event, downloadId: string) => {
      ...
      try {
        ...
        if (download.status === "paused") {
          ...
          const restoreData =
            download.metadata?.restoreData ||
            download.metadata?.persistedRestoreData;
          if (restoreData) {
            console.log(`使用保存的恢复数据恢复下载: ${downloadId}`);

            // 使用 restoreDownload 方法来恢复下载
            try {
              const restoredDownloadId =
                await this.downloadManager.restoreDownload({
                  app,
                  window: this.mainWindow!,
                  ...
                  callbacks: {
                    ...
                    onDownloadProgress: async ({
                      ...
                      if (
                        currentStatus &&
                        currentStatus.metadata?.lastUpdateTime
                      ) {
                        const timeDiff =
                          (currentTime -
                            currentStatus.metadata.lastUpdateTime) /
                          1000;
                        const bytesDiff =
                          currentBytes - currentStatus.bytesReceived;

                        if (timeDiff >= 1.0) {
                          downloadRate = bytesDiff / timeDiff;
                          const remainingBytes =
                            item.getTotalBytes() - currentBytes;
                          if (downloadRate > 0) {
                    ...
                    onDownloadInterrupted: async ({ id }) => {
                      ...
                      this.sendToRenderer("download-interrupted", {
                        id: downloadId,
                      });
                    },
                    onError: (error, data) => {
        ...
        const downloadId = await this.downloadManager.restoreDownload({
          ...
          callbacks: {
            ...
            onDownloadPersisted: async (data, restoreData) => {
              // 原生持久化回调 - 应用关闭时自动触发
              console.log(
                `下载 ${data.id} 已持久化，路径: ${restoreData.persistedFilePath}`
              );
    ...
    ipcMain.handle("get-download-status", (event, downloadId: string) => {
      try {
        ...
        return {
          ...
          status: downloadData.isDownloadInProgress()
            ? "downloading"
            : downloadData.isDownloadPaused()
              ? "paused"
              : downloadData.isDownloadCompleted()
                ? "completed"
                : downloadData.isDownloadCancelled()
                  ? "cancelled"
                  : downloadData.isDownloadInterrupted()
                    ? "interrupted"
                    : "pending",
    ...
    ipcMain.handle("get-all-downloads", () => {
      ...
      console.log(
        `获取所有下载任务，当前共 ${downloads.length} 个:`,
        downloads
      );
      return downloads;
    });
    ...
    ipcMain.handle(
      "delete-download",
      (event, downloadId: string, deleteFile: boolean = false) => {
        try {
          const download = this.downloads.get(downloadId);

          // 如果任务仍在进行中或等待中，先尝试取消底层下载
          if (
            download &&
            (download.status === "downloading" || download.status === "pending")
          ) {
            try {
              console.log(`删除前先取消正在进行的下载任务: ${downloadId}`);
              this.downloadManager.cancelDownload(downloadId);
              this.updateDownloadStatus(downloadId, { status: "cancelled" });
              this.sendToRenderer("download-cancelled", { id: downloadId });
            } catch (cancelError) {
              console.warn(
                "删除前取消下载任务失败，将继续删除本地记录:",
                cancelError
              );
            }
          }
          ...
          if (deleteFile) {
            // 删除文件 自己实现
            const downloadData =
              this.downloadManager.getDownloadData(downloadId);
            if (downloadData) {
