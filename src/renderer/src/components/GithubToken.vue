<template>
  <div v-if="showModal" class="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"
    @click="closeModal">
    <div
      ...
      <div class="p-4 space-y-4">
        <div class="space-y-2">
          ...
          <input id="token-input" v-model="tokenInput" type="password" placeholder="请输入您的 GitHub Token"
        ...
        <div class="bg-blue-50 p-3 rounded-lg">
          <p class="text-sm text-gray-700">
            ...
            <a href="https://github.com/settings/personal-access-tokens" target="_blank"
              class="text-blue-600 hover:text-blue-800 underline">GitHub Token 设置页面</a>
          </p>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 p-4 border-t border-gray-200">
        ...
        <button @click="confirmToken" :disabled="!tokenInput.trim()"
...
import { ref } from 'vue'
import { PluginGithub } from "@/core/plugin/PluginGithub"

const github = PluginGithub.getInstance();

const showModal = ref(false)
const tokenInput = ref('')
let tokenCallback: ((token: string) => void) | null = null

github.setTokenCallback((setToken) => {
...
function confirmToken() {
  if (tokenInput.value.trim() && tokenCallback) {
    tokenCallback(tokenInput.value.trim())
    closeModal()
  }
}
</script>
