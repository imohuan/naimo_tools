...
export class ElectronHotkeyBridge extends BaseSingleton {
  ...
  async registerGlobalHotkey(config: HotkeyConfig): Promise<boolean> {
    try {
      ...
      const success = await api.ipcRouter.windowRegisterGlobalHotkey(normalizedKeys, id)
  ...
  async unregisterGlobalHotkey(id: string): Promise<boolean> {
    try {
      ...
      const success = await api.ipcRouter.windowUnregisterGlobalHotkey(config.keys, id)
  ...
  async isGlobalHotkeyRegistered(keys: string): Promise<boolean> {
    try {
      const normalizedKeys = this.normalizeElectronKeys(keys)
      return await api.ipcRouter.windowIsGlobalHotkeyRegistered(normalizedKeys)
    } catch (error) {
  ...
  async clearAllGlobalHotkeys(): Promise<boolean> {
    try {
      ...
      for (const hotkeyId of hotkeyIds) {
        ...
        if (config) {
          const success = await api.ipcRouter.windowUnregisterGlobalHotkey(config.keys, hotkeyId)
          if (!success) {
            allSuccess = false
          }
        }
