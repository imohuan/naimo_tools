<template>
  <div class="h-full flex flex-col bg-gray-50">
    ...
    <div v-else class="flex-1 flex flex-col">
      <!-- 保存反馈提示 -->
      <div v-if="saveFeedback.show" :class="[
        ...
        <span :class="saveFeedback.type === 'success' ? 'text-green-500' : 'text-red-500'">
          {{ saveFeedback.type === 'success' ? '✅' : '❌' }}
        </span>
        <span class="flex-1 text-sm">{{ saveFeedback.message }}</span>
        <button @click="saveFeedback.show = false" class="text-gray-400 hover:text-gray-600 transition-colors">
          ✕
        </button>
      </div>

      <!-- 无插件设置提示 -->
      <div v-if="pluginSettingsList.length === 0" class="flex-1 flex items-center justify-center">
      ...
      <!-- 顶部区域：操作按钮 -->
      <div v-if="pluginSettingsList.length > 0" class="px-3 py-2 bg-white border border-gray-200 rounded-lg ">
        <div class="flex items-center justify-between gap-3">
          <div class="flex items-center gap-2">
            <span class="text-sm font-medium text-gray-700">插件设置管理</span>
            <span class="text-xs text-gray-500">({{ pluginSettingsList.length }} 个插件)</span>
          </div>
          <div class="flex items-center gap-2">
      ...
      <div v-if="pluginSettingsList.length > 0" class="flex-1 pt-2 flex flex-col pb-2">
        <div class="grid grid-cols-1 gap-2">
          <div v-for="pluginSetting in pluginSettingsList" :key="pluginSetting.pluginId"
                ...
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-3">
                    <h3 class="text font-semibold text-gray-900">
                      ...
                      <span
                        class="bg-gray-100 px-2 py-1 rounded text-xs transform scale-75 origin-center-left inline-block">v1.0.0</span>
                    </h3>
                  </div>
                  <!-- 折叠/展开图标 -->
            ...
            <!-- 设置项 -->
            <div v-show="!isCollapsed(pluginSetting.pluginId)"
              class="overflow-hidden transition-all duration-300 ease-in-out px-2">
              <div class="space-y-1">
                <SettingItem v-for="setting in pluginSetting.settings" :key="setting.name" :setting="setting"
                  :plugin-id="pluginSetting.pluginId" :value="settingValues[pluginSetting.pluginId][setting.name]"
                  @update:value="updateSettingValue(pluginSetting.pluginId, setting.name, $event)" />
              </div>
            </div>
          </div>
        </div>
...
const getPluginSettings = async () => {
  try {
    ...
    // 获取所有已安装的插件
    const installedPlugins = Array.from(pluginManager.installedPlugins.values())
    console.log('🔍 已安装的插件数量:', installedPlugins.length)
    console.log('🔍 已安装的插件列表:', installedPlugins.map(p => ({ id: p.id, name: p.name, hasSettings: !!p.settings, settingsCount: p.settings?.length || 0 })))

    // 从存储中加载已保存的插件设置
    const savedSettings = await storeBridge.get('pluginSettings') as Record<string, Record<string, any>> || {}
      ...
      if (plugin.settings && plugin.settings.length > 0) {
        ...
        for (const setting of plugin.settings) {
          if (settingValues.value[plugin.id][setting.name] === undefined) {
            // 优先使用已保存的设置值
            if (savedSettings[plugin.id] && savedSettings[plugin.id][setting.name] !== undefined) {
              settingValues.value[plugin.id][setting.name] = savedSettings[plugin.id][setting.name]
            } else {
              // 使用默认值
...
const saveAllSettings = async () => {
  try {
    ...
    // 获取当前已保存的所有插件设置
    const currentPluginSettings = await storeBridge.get('pluginSettings') as Record<string, Record<string, any>> || {}
    ...
    if (success) {
      ...
      showSaveFeedback(true, `所有插件设置保存成功 (${Object.keys(settingValues.value).length} 个插件)`)
    } else {
...
const resetAllSettings = async () => {
  try {
    ...
    for (const pluginSetting of pluginSettingsList.value) {
      const plugin = Array.from(pluginManager.installedPlugins.values()).find(p => p.id === pluginSetting.pluginId)
      if (plugin && plugin.settings) {
        for (const setting of plugin.settings) {
          settingValues.value[pluginSetting.pluginId][setting.name] = getDefaultValue(setting)
        }
      }
    }
    ...
    showSaveFeedback(true, `所有插件设置已重置为默认值 (${pluginSettingsList.value.length} 个插件)`)
  } catch (error) {
