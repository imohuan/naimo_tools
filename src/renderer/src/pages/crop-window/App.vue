<template>
  <div class="relative w-full h-full bg-transparent" ref="containerRef">
    ...
    >
      <!-- 背景遮罩 -->
      <div
        class="absolute bg-gray-950 opacity-50 pointer-events-none z-10"
        :style="maskTopStyle"
      ></div>
      <div
        class="absolute bg-gray-950 opacity-50 pointer-events-none z-10"
      ...
      <!-- 选择区域 -->
      <div
        class="absolute border-2 border-blue-500 bg-transparent pointer-events-auto cursor-move min-w-5 min-h-5 z-20"
      ...
      >
        ...
        <div
          ...
          :key="handle.position"
          :class="[
            'absolute w-3 h-3 bg-blue-500 border-2 border-white rounded-full pointer-events-auto z-30',
            handle.position,
          ]"
    ...
    >
      <div class="flex items-center gap-3 mb-2">
        <span
          class="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-mono"
          >W/A/S/D</span
        >
        <span>移动鼠标光标</span>
      </div>
...
const toolbarStyle = computed(() => {
  ...
  const toolbarWidth =
    padding * 2 + buttonSize * buttonCount + gap * 4 + separatorWidth;
  ...
  if (left < 0) {
    left = Math.max(
      controlOffset,
      selection.x + selection.width - toolbarWidth - controlOffset
    );
  ...
  if (top < 0) {
    top = Math.max(
      controlOffset,
      selection.y + selection.height - toolbarHeight - controlOffset
    );
    left -= controlOffset;
