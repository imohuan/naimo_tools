<template>
  <div class="h-full flex flex-col bg-gray-50">
    ...
    <div v-show="showAddDownloadDialog" class="h-full flex flex-col">
      <AddDownloadDialog
        @close="showAddDownloadDialog = false"
        @add="handleAddDownload"
      />
    </div>
  </div>
</template>
...
const tabs = [
  {
    id: "all",
    label: "全部下载",
    count: computed(() => downloads.value.length),
  },
  {
    id: "downloading",
    label: "下载中",
  ...
  },
  {
    ...
    label: "等待中",
    count: computed(
      () => downloads.value.filter((d) => d.status === "pending").length
    ),
  },
  {
    id: "failed",
    label: "失败",
    count: computed(
      () => downloads.value.filter((d) => d.status === "error").length
    ),
  },
];
...
const setupEventListeners = () => {
  ...
  // 监听下载持久化
  (naimo.download as any).onDownloadPersisted(
    (data: { id: string; persistedFilePath: string }) => {
